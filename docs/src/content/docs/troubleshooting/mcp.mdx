...
Symptoms against `https://<worker-url>/mcp` and the OAuth flow. Enablement steps: [Enable MCP](/mcp/enable).
...
**Fix:** Set all three, redeploy ([Enable MCP](/mcp/enable)). Generate the cookie key with `openssl rand -hex 32`.

## Cookie / session signing failures

**Cause:** `COOKIE_ENCRYPTION_KEY` signs OAuth cookies and related state. Wrong, rotated, or empty values break the browser leg of auth (callback, CSRF cookie mismatch, “invalid state”).
...
**Fix (in tree):** OAuth provider construction uses a **guarded dynamic import** with a fallback, and builds the provider at request time inside workerd (same idea as Alchemy’s own workers shim). Stay on current `remote-db/cloudflare` sources; don’t reintroduce a top-level static import of the provider package.
...
**Fix:** Call `memory_size` before long writes; trim `content` / `context` / `tags`. See [Tools → Embedding size budget](/mcp/tools#embedding-size-budget).
