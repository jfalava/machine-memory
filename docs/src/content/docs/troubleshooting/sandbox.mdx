...
2. In the sandbox, connect the MCP client to `https://<worker-url>/mcp`. A browser on that machine can finish OAuth directly. A headless client prints a code; enter it at `https://<worker-url>/activate` from a browser you already have. See [Enable MCP](/docs/mcp/enable).
