...
| ------------------ | ------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **MCP** `/mcp`     | GitHub OAuth 2.1 in the browser | No `MACHINE_MEMORY_DB_TOKEN`. Client must complete OAuth; cookie signing uses `MACHINE_MEMORY_COOKIE_ENCRYPTION_KEY` on the Worker. |
...
1. Worker already [provisioned](/docs/mcp/infrastructure) and [MCP enabled](/docs/mcp/enable) from a machine that can deploy.
