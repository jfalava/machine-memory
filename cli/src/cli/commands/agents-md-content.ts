...
const AGENTS_MD_BACKEND_CONTENT = {
  ...
  mcp: [
    ...
    "All memory access goes through MCP tools: `list_repositories`, `memory_suggest`, `memory_query`, `memory_get`, `memory_list`, `memory_verify`, `memory_diff`, `memory_size`, `memory_add`, `memory_update`, `memory_delete`.",
    ...
    'Search policy: `memory_suggest` with `{ repository, files }` for file-scoped lookup; `memory_query` with `mode: "keyword"` for exact names, paths, commands, and identifiers, `mode: "semantic"` when the same concept may use different wording, `mode: "hybrid"` (default) for broad investigation. Filter with `tags` (substring) / `status` / `memory_type` / `certainty` and cap with `limit` (default 8, max 50). D1 records are canonical and Vectorize is only a retrieval index.',
    "Writes (`memory_add`, `memory_update`, `memory_delete`) are permanent. `memory_add` echoes `written_to` plus the new `id` and `potential_conflicts`; `memory_delete` echoes `deleted_from` with `deleted` / `existed`. Embeddings re-sync to Vectorize automatically on add/update.",
...
const AGENTS_MD_MCP_WORKFLOW = [
  ...
  "- Broad audit: `memory_list` with `{ repository }` (filter by `tags` / `memory_type` / `status` / `certainty`) or `memory_query` with tag-oriented keywords",
  ...
  "7. Keep status hygiene. Status memories are for transient progress, should set `expires_after_days` (at creation or via `memory_update`), and should be updated rather than duplicated. Deprecate via `memory_update` with `status` (and `superseded_by` when replaced).",
  "8. Separate durable and transient facts. Use `decision`, `reference`, or `gotcha` for reusable knowledge; use `status` only for short-lived snapshots.",
