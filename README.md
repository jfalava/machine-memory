...
Add a concise, idempotent memory policy to your project's `AGENTS.md`. The command creates the file when needed and replaces its managed block:

```sh
machine-memory update-agents-md
```

Use the generated block as the project policy; detailed command examples follow in the Usage section.

## Usage

Default output is JSON — designed to be parsed by an LLM agent.
`--brief` emits compact text lines for human scanning; `--json-min` emits IDs plus compact candidate summaries for agent retrieval.
Use `--limit <n>` to bound query, list, suggest, and sweep results.
...
- `query <search_term>`
  - Flags: `--tags`, `--type`, `--certainty`, `--include-deprecated`, `--limit`, `--brief`, `--json-min`, `--quiet`
  - Returns ranked matches with `score`; empty results return diagnostics/hints
- `list`
  - Flags: `--tags`, `--type`, `--certainty`, `--status`, `--include-deprecated`, `--limit`, `--brief`, `--json-min`, `--quiet`
- `get <id|id,id,...>`
  - Batch requests return `{ results, missing_ids? }`
...
- `suggest --files "<csv paths>"`
  ...
  - Flags: `--tags`, `--type`, `--certainty`, `--include-deprecated`, `--limit`, `--brief`, `--json-min`, `--quiet`
  - Derives keywords from exact file paths, mapped tags, and path-neighborhood matches
- `sweep --files "<csv paths>"`
  - Optional `--query`, `--tags`, and `--limit`; diagnostic aggregation of suggest/query/list
- `verify <id> <fact>`
